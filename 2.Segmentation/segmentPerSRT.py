...
for filePath in listFile:
    ...
    for line in srtOriginal:
        ...
        if flgGesture:
        	outSegment = cv2.VideoWriter(outputVideoPath+inputName+'/'+ line.text.upper()+'_'+str(sentence)+'.mp4', fcc, fpsOutput, (videoWidth, videoHeight))
        else:
        	
        	#foldName = outputVideoPath+inputName+'/'+inputName+'_'+str(sentence)+'.mp4'
        	#print(foldName)
        	outSegment = cv2.VideoWriter(outputVideoPath+inputName+'/'+str(sentence)+'.mp4', fcc, fpsOutput, (videoWidth, videoHeight))
