...
def getCleanedData():
    df = pd.read_pickle("./Data/AEC_cleaned/data_10_10_24.pk")

    wordList = pd.read_csv("./Data/list18.csv", header=None)
    ...
    for pos in range(len(df[names])):
        ...
        index = labelIndex.index(df[labels][pos])
        label = df[labels][pos]
        name = df[names][pos]
    ...
    return X_train, Y_train, meaning

class SignLanguageDataset(torch.utils.data.Dataset):

    def __init__(self, src_file, x, y, y_labels, datasetType="train", split=0.8):
        ...
        if self.datasetType == "train":
            ...
            X_test = np.asarray(x) #LoadData.getKeypointsfromIdList(args.keypoints_input_Path, x, bodyParts, timestepSize)
...
def main():
    ...
    x, y, y_labels = getCleanedData()

    X_train, X_test, y_train, y_test = train_test_split(x, y, train_size=split , random_state=42, stratify=y)

    dataTrainXY = SignLanguageDataset(src, X_train, y_train, y_labels, datasetType="train",split=split)
    dataTestXY = SignLanguageDataset(src, X_test, y_test, y_labels, datasetType="test",split=split)
